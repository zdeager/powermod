...
**Routing (hardware/router.py) — EXPERIMENTAL, do not fabricate this state.** A grid A* autorouter (escape stubs for fine-pitch, neck-down at pads, spatial-hashed obstacles, KiCad's own DRC ratsnest as the target list) currently routes **111 of 115 ratsnest edges**. Honest state per `kicad-cli pcb drc`: **4 edges unrouted (CC1_OUT, CC2_OUT, LED_BAT_A, SCL), 6 shorts and 23 clearance violations remain** — root causes identified (the solder-jumper footprints use custom-shape pads whose true copper exceeds the parsed bbox; one via-vs-track clearance edge case) but not yet fixed. The router found and fixed six systematic bug classes under DRC iteration; these are the seventh and eighth. Finish interactively in pcbnew (rip the flagged nets, route the 4 stragglers) or harden router.py further.
