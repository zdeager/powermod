...
|---|---|---|---|---|---|---|
| U1 | **BM8563ESA** (SOP-8) | SOP-8 (= SOIC-8, 1.27mm) | [C269877](https://www.lcsc.com/product-detail/C269877.html) | 1 | **$0.108** | RTC, PCF8563-class. **Reversed from RV-3028 (−$1.67)**. ⚠ **FITTED PART = C269877 (SOP-8), not C194063.** The Belling C194063 is the **TSSOP-8** die (0.65mm pitch) — it does **not** fit this board's SOIC-8 land and was OOS at check; C269877 matches the footprint (~80k JLC stock). Both are gate-verified PCF8563-class (claims row 44). |
...
- **U1 BM8563: FITTED = GATEMODE C269877 (SOP-8, $0.108)** — chosen over Belling C194063 on **package**, not price: C194063 is the TSSOP-8 (0.65mm) die and does not fit this board's SOIC-8 (SOP-8, 1.27mm) land, and was OOS at check. C269877 matches the footprint with ~80k JLC stock. Both are gate-verified from their own datasheets (claims row 44, closed 2026-07-15 — electrical tables byte-identical, NXP PCF8563 as the architecture reference). If a SOP-8 Belling BM8563 is later stocked it is a drop-in alternate; the TSSOP-8 C194063 is **not** — the former sole-source risk is retired but watch the package.
...
## 8. Design risks — conscious sign-off before ordering (2026-07-19 audit)

None of these block fabrication; they are judgment calls the design makes on purpose. Listed so the yes/no is deliberate, not accidental.

1. **RTC accuracy is the product's soft spot (make this a conscious accept).** BM8563 is **uncompensated**: ~±20 ppm initial + a cold parabola (~22 ppm @ 0°C, ~43 ppm @ −10°C) — worse than Witty Pi's ±2 ppm *compensated*, on the exact axis (wake-on-time, outdoors) this product exists for. Firmware can compensate *wake times* arithmetically (BM8563 has no offset register, so the free-running clock is not trimmable). If drift ever matters, the recorded fix is a **PCF85063A-class trimmable RTC (+$0.43)**. See `powermod-spec.md` §RTC.
2. **No battery reverse-polarity protection — by design.** A reversed JST cell **destroys the TP4056** (no series FET/fuse). Acceptable only if the J3 pigtail polarity is keyed/labelled unambiguously (vendors wire both ways — schematic checklist item 10). Cheap hardening if wanted: a series P-FET or a fuse on VBAT.
   - **⚠️ Escalation identified 2026-07-21 — the worst case on the board, and it is not the single-cell one.** `J3` and `TP2` (`BAT_PAD`) are the **same `VBAT` net**, i.e. two connection points on one node. A user who attaches a cell to each has paralleled two cells with no fuse, balancing or isolation. Two consequences, both worse than risk #2 alone: (a) *correct* polarity but mismatched charge dumps cell-into-cell at **6A+ for minutes** through a 2A connector, bypassing the charger's preconditioning entirely; (b) **one cell reversed is a dead short through both cells** — ~8.4V across a few hundred mΩ, order **40A** — a venting/fire event rather than a destroyed IC. Two connection points also double the odds of a polarity error. **Documented in the user guide (FAQ 5.5, "One cell, one connection point").** This raises the value of the hardening above: a series P-FET or fuse on `VBAT` now mitigates *two* fire paths, not one, and is **the single highest-value change for a rev B**.
3. **No ESD/surge protection on the external connectors.** USB-C VBUS and the STEMMA-QT I²C lines reach silicon directly. For field plug/unplug, a VBUS TVS and an I²C ESD array are cheap production-hardening (optional).
4. **Battery-path FET margin is thin at cell-empty.** Q2 (AON7407, needs ~−2.5V Vgs) sees Vgs ≈ −2.7V at the 3.0V cutoff with VSYS ≈ 2.7V — turns on, but with little margin and rising Rds(on) exactly when it carries load. Bench-verify the Q1↔Q2 handover at the low-voltage cutoff (already on the §7 list — this is the acceptance criterion).
5. **SWD shares the two BAT-LED pins** (SWDIO/SWCLK): field re-flash needs connect-under-reset (NRST on J5), and must not be done with a deeply-discharged cell attached (programmer 3.3V back-feeds through the LDO). Documented; workable.

**Not a risk — recorded so it isn't "fixed" by mistake:** the board is **65×40 mm, not the 65×30 mm Witty Pi outline**. This is **intentional** — the extra 10 mm carries the user-accessible interface (button, LEDs, STEMMA-QT, test/jumper pads) *outside* the Witty Pi footprint, overhanging on top, so it stays reachable when stacked. The Pi Zero 58×23 mm mounting pattern is preserved. Do not "correct" the outline to 65×30.
